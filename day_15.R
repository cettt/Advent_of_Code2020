...
run_day15 <- function(n, input) {
  tab_t <- rep(0, n) #tab_t[x] = t means that the last time x-1 has been said was at time t
  tab_t[input + 1] <- seq_along(input)
  y <- 0 #first number after the input sequence is zero

  for (turn in (length(input) + 1):n) {
    y2 <- turn - tab_t[y + 1] #y2 candidate for the next number in the sequence (after y)
    tab_t[y + 1] <- turn #update tab_t
    y <- y2 * (y2 != turn) #if y2 == turn then y had never been said before in which case y <- 0
  }
  return(which(tab_t == n) - 1)
}
