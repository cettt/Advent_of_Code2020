data23 <- as.numeric(strsplit(as.character(read.table("Input/day23.txt")[,1]), "")[[1]])
...
play_cups <- function(rounds, input = data23, n = 1e6L, part1 = TRUE) {
  ...
  for (s in seq_len(rounds)) {
    ...
    dest <- if (cup0 == 1L) n else cup0 - 1L #this is faster than using %%
    while (dest %in% c(cup1, cup2, cup3)) dest <- if (dest == 1L) n else dest - 1L
...
#part 2---------
print(play_cups(rounds = 1e7, data23, n = 1e6L, part1 = FALSE), 16) #about 16 seconds
